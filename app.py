...
def checkNumber():
    ...
    if not number == int or float:
        checkNumber = "Input is not a number"

    if int(number)%2 == 0:
        checkNumber = "Number is even"

    if int(number)%2 != 0:
        checkNumber = "Number is odd"

    if  number == "":
        checkNumber = "No number was entered"

    

    return render_template('result.html', checkNumber=checkNumber)
